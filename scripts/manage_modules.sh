...
# ---- Interactive module picker (single selection) -----------------------
...
choose_module() {
  ...
  while true; do
    read -r -p "Enter module number (1-${n}): " ans
    ans=$(echo "$ans" | tr -d '[:space:]')

    if [[ -z "$ans" || "${ans,,}" == "q" || "$ans" == "$((n + 1))" ]]; then
      return 1
    fi
...
main() {
  ...
  # Step 4: pick ONE module
  if ! choose_module new; then
    echo "No module selected. Exiting."
    exit 0
  fi
  local mod_name="${new[0]}"
  echo

  # Step 5: process the module
  box "INSTALLING: ${mod_name%.json}"

  mkdir -p "$MODULES_DIR"
  do_download "$mod_name" || exit 1
  ...
  if yesno "  Generate a keep JSON file for this module? [y/N]:"; then
    ...
    if [[ ${#lines[@]} -eq 0 ]]; then
      ...
      keep_file="${KEEP_DIR}/keep_${base}.json"
      mkdir -p "$KEEP_DIR"
      generate_keep "$base" "$attr" "$op" "$val" "$keep_file"

      # Extract first SNOMED code from codes for config.yaml hint
      if [[ -n "$codes" ]]; then
        # codes looks like "233678006 | 195967001" -- take first numeric code
        detect_snomed=$(echo "$codes" | grep -oP '\d{5,}' | head -1)
      fi
    fi
  fi

  # ---- Print config.yaml changes ----
  box "CONFIG.YAML CHANGES REQUIRED"
  echo "  The following lines in config.yaml must be updated to use this module:"
  echo

  local base="${mod_name%.json}"
  echo "  # --- Patient simulation (stage 2) -------------------------------------------"
  echo "  simulation:"
  if [[ -n "$keep_file" ]]; then
    echo "    keep_module: \"config/keep_${base}.json\""
  else
    echo "    keep_module: \"config/keep_ra.json\"   # <-- REPLACE with a keep file for '${base}'"
  fi
  if [[ -n "$detect_snomed" ]]; then
    echo "    disease_snomed_code: \"${detect_snomed}\"   # ${base}"
  else
    echo "    disease_snomed_code: \"69896004\"   # <-- REPLACE with the SNOMED code for '${base}'"
  fi
  echo

  box "DONE"
  echo "  Module installed:  config/modules/${mod_name}"
  if [[ -n "$keep_file" ]]; then
    echo "  Keep file:         ${keep_file}"
  fi
  echo
  echo "  Re-run this script to download additional modules (one at a time)."
}
